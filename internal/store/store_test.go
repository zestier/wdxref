...
func TestNoChangelogUpsert(t *testing.T) {
	...
	entries, err := w.rdb.XRange(context.Background(), "changelog", "-", "+").Result()
	if err != nil {
		t.Fatalf("XRange: %v", err)
	}
	if len(entries) != 0 {
		t.Errorf("expected 0 changelog entries with NoChangelog, got %d", len(entries))
	}
}

func TestNoChangelogDelete(t *testing.T) {
	w := newTestWriter(t)
	...
	// Only the original upsert entry should be in changelog (no delete).
	entries, err := w.rdb.XRange(context.Background(), "changelog", "-", "+").Result()
	if err != nil {
		t.Fatalf("XRange: %v", err)
	}
	if len(entries) != 1 {
		t.Errorf("expected 1 changelog entry (upsert only), got %d", len(entries))
	}
	if entries[0].Values["m"] == "" {
		t.Errorf("changelog entry has empty mappings, expected upsert entry")
	}
}
...
func TestNoChangelogUpsertStillDiffsIndexes(t *testing.T) {
	...
	// Old index should be removed, new one present.
	oldVal, err := w.rdb.HGet(context.Background(), "p:213", "old").Result()
	if err == nil && oldVal != "" {
		t.Errorf("expected old index to be removed, got %q", oldVal)
	}

	newVal, err := w.rdb.HGet(context.Background(), "p:213", "new").Result()
	if err != nil {
		t.Fatalf("HGet new: %v", err)
	}
	if newVal != "42" {
		t.Errorf("p:213 new = %q, want 42", newVal)
	}
